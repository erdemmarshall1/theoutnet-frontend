...
const loadMessages = async () => {
  const res = await get('/home/userKefu/getMsgList', { shopId: route.query.shopId })
  if (res?.data) { messages.value = res.data; nextTick(() => { if (chatBox.value) chatBox.value.scrollTop = chatBox.value.scrollHeight }) }
}

const sendMsg = async () => {
  if (!text.value.trim()) return
  await qe(post('/home/userKefu/sendMsg', { content: text.value, shopId: route.query.shopId }))
  text.value = ''
  loadMessages()
}

const handleNewMessage = (msg) => {
  if (msg.shopId === route.query.shopId) {
...
onMounted(() => {
  if (!route.query.shopId) return ElMessage.error('Missing shop ID')
  loadMessages()
  markRead({ sessionId: `shop_${route.query.shopId}`, userId: store.userInfo?._id })
